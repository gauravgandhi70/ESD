...
Note:
1.The SDA and SCL lines are connected to P0.2 and P3.4
...
{
	delay_us(1);
	P3_4 = 1 ;		// Wait for Some time and Pull the SCL line High

	delay_us(1);        // Wait for Some time
	P3_4 = 0;		// Pull back the SCL line low to Generate a clock pulse
}
...
void I2C_Start()
{
	...
	P3_4 = 1;		//Pull SCL high
	delay_us(1);

	P1_2 = 0;        //Now Pull SDA LOW, to generate the Start Condition
	delay_us(1);

	P3_4 = 0;        //Finally Clear the SCL to complete the cycle
	...
	for(i=0;i<8;i++)		// loop 8times read 1-byte of data
	 {
		...
		P3_4 = 0;			// Clear SCL to complete the Clock
	   }
