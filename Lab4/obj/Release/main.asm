...
; This file generated Thu Nov 24 04:54:17 2016
;--------------------------------------------------------
	.module main
...
;	-----------------------------------------
_main:
;	main.c:42: char store,c[4],d[5],name[3]="012",b[5];        // c,d,b are used for taking input from the user
;	genPointerSet
;     genFarPointerSet
...
;     genFarPointerSet
	...
	mov	a,#0x32
	movx	@dptr,a
;	main.c:47: lcd_init();                                     // Initilaize LCD
;	genCall
	lcall	_lcd_init
;	main.c:48: uart_init()	;                                   // Initilalize UART
;	genCall
	lcall	_uart_init
;	main.c:49: timer_init();                                   // Initialize the Timer
;	genCall
	lcall	_timer_init
;	main.c:52: P1_0=0;
;	genAssign
	clr	_P1_0
;	main.c:53: eereset();                                      // EEPROM is reset at every powerup
;	genCall
	lcall	_eereset
;	main.c:56: lcdgotoxy(4,1);
;	genAssign
...
;	genCall
	mov	dpl,#0x04
	lcall	_lcdgotoxy
;	main.c:57: lcdputstr("Count-0");
;	genCall
;	Peephole 182.a	used 16 bit load of DPTR
	mov	dptr,#__str_1
	mov	b,#0x80
	lcall	_lcdputstr
;	main.c:59: lcdgotoxy(4,15);
;	genAssign
	mov	dptr,#_lcdgotoxy_PARM_2
	mov	a,#0x0F
	movx	@dptr,a
...
;	genCall
	mov	dpl,#0x3A
	lcall	_lcdputch
;	main.c:63: IOEX_WriteByte(0x01);
;	genCall
	mov	dpl,#0x01
	lcall	_IOEX_WriteByte
;	main.c:66: for(i=0;i<3;i++)
;	genAssign
	mov	r2,#0x00
	mov	r3,#0x00
00294$:
...
;	genAssign
	clr	_EA
;	main.c:69: countdown_alarm(0,0,0,2,i);     // Display Initial Status of the C/D timer as 0.0.0
;	genAssign
...
;	Peephole 108.d	removed ljmp by inverse jump logic
	jnb	_RI,00292$
;	Peephole 300	removed redundant label 00414$
;	main.c:95: store=getchar();
;	genCall
	lcall	_getchar
;	main.c:96: putchar(store);
;	genCall
	mov  r2,dpl
;	Peephole 177.a	removed redundant mov
	...
	lcall	_putchar
	pop	ar2
;	main.c:97: if(store=='1'){printf_tiny("\n\n\r\t\t ******  Stop Clock  *****"); }
;	genCmpEq
;	gencjne
;	gencjneshort
...
;	genCall
	...
	ljmp	00164$
00163$:
;	main.c:98: else if(store=='2' && timers<2){printf_tiny("\n\n\r\t\t ******  Restart Clock  ******"); }
;	genCmpEq
;	gencjneshort
...
;	genCall
	...
	ljmp	00164$
00159$:
;	main.c:99: else if(store=='3' && timers<2){printf_tiny("\n\n\r\t\t ******  Reset Clock  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00155$:
;	main.c:100: else if(store=='4'&& timers<2){printf_tiny("\n\n\r\t\t ******  Load Counter Value ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00151$:
;	main.c:101: else if(store=='5' && timers<2){printf_tiny("\n\n\r\t\t ******  Timer Enable  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00147$:
;	main.c:102: else if(store=='6' && timers<2){printf_tiny("\n\n\r\t\t ******  Timer Disable  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00143$:
;	main.c:103: else if(store=='7' && timers<2){printf_tiny("\n\n\r\t\t ******  Custom Character  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00139$:
;	main.c:104: else if(store=='x' && timers<2){printf_tiny("\n\n\r\t\t ******  Reset Counter  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00135$:
;	main.c:105: else if(store=='i' && timers<2){printf_tiny("\n\n\r\t\t ******  Configure IO Pins  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00131$:
;	main.c:106: else if(store=='s' && timers<2){printf_tiny("\n\n\r\t\t ******  Status of IO Expander  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00127$:
;	main.c:107: else if(store=='w' && timers<2){printf_tiny("\n\n\r\t\t ******  EEPROM Write  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00123$:
;	main.c:108: else if(store=='r' && timers<2){printf_tiny("\n\n\r\t\t ******  EEPROM Read  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00119$:
;	main.c:109: else if(store=='c' && timers<2){printf_tiny("\n\n\r\t\t ******  CGRAM Dump  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genCall
	...
	ljmp	00164$
00115$:
;	main.c:110: else if(store=='d' && timers<2){printf_tiny("\n\n\r\t\t ******  DDRAM Dump  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	Peephole 112.b	changed ljmp to sjmp
	sjmp	00164$
00111$:
;	main.c:111: else if(store=='h' && timers<2){printf_tiny("\n\n\r\t\t ******  Hex Dump  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	Peephole 112.b	changed ljmp to sjmp
	sjmp	00164$
00107$:
;	main.c:112: else if(store=='l'){printf_tiny("\n\n\r\t\t ******  LCD DISPLAY  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	Peephole 112.b	changed ljmp to sjmp
	sjmp	00164$
00104$:
;	main.c:113: else if(store=='q'){printf_tiny("\n\n\r\t\t ******  Clear LCD  ******"); }
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	genAssign
	...
	ljmp	00235$
00234$:
;	main.c:124: else if(store=='2' && EA==0)
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;     genFarPointerSet
	...
	mov	a,r0
	movx	@dptr,a
;	main.c:164: if(nsec[num]>59){printf_tiny("\n\n\r *-ERROR-*\n\r\t Enter a valid number between 0 to 59");}
;	genPointerGet
;	genFarPointerGet
	mov	dpl,_main_sloc0_1_0
	mov	dph,(_main_sloc0_1_0 + 1)
...
;	genIfxJump
	...
	ljmp	00182$
00479$:
;	main.c:178: countdown_alarm(nms[num],nsec[num],nmi[num],2,num);
;	genIpush
	push	ar2
;	genPlus
;	Peephole 236.g	used r5 instead of ar5
...
;	genAssign
	clr	_EA
;	main.c:208: lcdgotoxy(num+1,1);  // Print Disabled on LCD
;	genCast
	mov	ar5,r3
;	genPlus
;     genPlusIncr
...
;	genCall
	...
	pop	ar3
	pop	ar2
;	main.c:211: delay_ms(500);      // Wait for 500 ms
;	genCall
;	Peephole 182.b	used 16 bit load of dptr
	mov	dptr,#0x01F4
	push	ar2
	...
	pop	ar3
	pop	ar2
;	main.c:212: cnt[num]=0;         // Disable the counter by putting 0 in cnt array
;	genPlus
;	Peephole 236.g	used r3 instead of ar3
	mov	a,r3
	add	a,#_cnt
...
;	Peephole 182.a	used 16 bit load of DPTR
	...
	pop	ar3
	pop	ar2
;	main.c:241: num=atoh_data(c);// atoh_data function converts ascii values to the HEX numbers
;	genCall
;	Peephole 182.a	used 16 bit load of DPTR
	mov	dptr,#_main_c_1_1
	mov	b,#0x00
...
;	genIfxJump
	...
	ljmp	00192$
00490$:
;	main.c:246: row_val[i]=num;         // Store data from user into row_val[] array
;	genPlus
;	Peephole 236.g	used r4 instead of ar4
	mov	a,r4
...
;	genCall
	...
	pop	ar3
	pop	ar2
;	main.c:249: for(j=0;j<=i;j++)          // This fop loop is used for displaying the contents of the ROW_val on the screen
;	genAssign
	mov	ar6,r4
	mov	ar7,r5
;	genAssign
...
;	Peephole 236.b	used r7 instead of ar7
	...
	ljmp	00306$
00309$:
;	main.c:260: lcdcreatechar(ccode,row_val);           // After getting all 7 values, new character  is storedin CGRAM
;	genCast
...
;	Peephole 182.a	used 16 bit load of DPTR
	...
	mov	r4,dph
	pop	ar2
;	main.c:272: if(num>7){printf_tiny("\n\n\r *-ERROR-*\n\r\t Enter a valid number between 0 to 7");}
;	genCmpGt
;	genCmp
	clr	c
	mov	a,#0x07
...
;	main.c:274: ccode=num;
;	genCast
;	main.c:275: lcd_dis_cus(ccode,2,3);
;	genAssign
...
;	main.c:293: DDRAM_dump();
;	genCall
	push	ar2
	lcall	_DDRAM_dump
	pop	ar2
00235$:
;	main.c:296: if(store=='w' || store=='r' || store=='l')
;	genCmpEq
;	gencjne
;	gencjneshort
;	Peephole 241.d	optimized compare
...
;	Peephole 182.a	used 16 bit load of DPTR
	...
	pop	ar3
	pop	ar2
;	main.c:305: addr=atoh(b);            // Get data from the user    //Conver that data into hex
;	genCall
;	Peephole 182.a	used 16 bit load of DPTR
	mov	dptr,#_main_b_1_1
	mov	b,#0x00
	...
	pop	ar3
	pop	ar2
;	main.c:306: if(addr<2048)              // Check if address is in valid range
;	genAssign
	mov	ar6,r4
	mov	ar7,r5
;	genCmpLt
...
;	main.c:317: page=addr/256;          // Calculate PAage block from the address
;	genAssign
	mov	ar6,r4
	mov	ar7,r5
;	genRightShift
...
;	genAssign
	...
	inc	dptr
	movx	@dptr,a
;	main.c:326: gets(d);            // Get data from the user
;	genCall
;	Peephole 182.a	used 16 bit load of DPTR
	mov	dptr,#_main_d_1_1
	mov	b,#0x00
	...
	pop	ar4
	pop	ar2
;	main.c:327: dat=atoh_data(d);   //Conver that data into hex
;	genCall
;	Peephole 182.a	used 16 bit load of DPTR
	mov	dptr,#_main_d_1_1
	mov	b,#0x00
...
;	genIfxJump
	...
	ljmp	00245$
00518$:
;	main.c:339: EEPROM_WriteByte((addr-page*256),dat,page);     // Write byte to the specified address
;	genAssign
	mov	ar6,r4
;	genCast
;	genCast
...
;	main.c:346: rd=EEPROM_ReadByte((addr-page*256),page);
;	genAssign
	mov	ar3,r4
	mov	ar6,r5
;	genCast
...
00521$:
	ljmp	00284$
00522$:
;	main.c:351: rd=EEPROM_ReadByte((addr-page*256),page);
;	genAssign
;	genCast
;	genCast
	mov	dptr,#_EEPROM_ReadByte_PARM_2
...
;	main.c:400: eeprom_profile();
;	genCall
	push	ar2
	lcall	_eeprom_profile
	pop	ar2
;	Peephole 112.b	changed ljmp to sjmp
	sjmp	00284$
00262$:
;	main.c:405: else if(store=='b')
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;	main.c:407: eeprom_block_fill();
;	genCall
	push	ar2
	lcall	_eeprom_block_fill
	pop	ar2
00284$:
;	main.c:412: if(store=='m')
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.b	optimized misc jump sequence
...
;------------------------------------------------------------
;------------------------------------------------------------
;	main.c:447: void ext_zero() interrupt 0
;	-----------------------------------------
;	 function ext_zero
;	-----------------------------------------
_ext_zero:
	...
	push	psw
	mov	psw,#0x00
;	main.c:450: io_counter++;               // Increament the counter after every button press
;	genAssign
	mov	dptr,#_io_counter
	movx	a,@dptr
	mov	r2,a
...
;	Peephole 236.b	used r3 instead of ar3
	...
	inc	dptr
	movx	@dptr,a
;	main.c:451: if(io_counter==16)          // After 16 counts reet the counter to 0;
;	genAssign
	mov	dptr,#_io_counter
	movx	a,@dptr
	mov	r2,a
...
;	genAssign
	...
	movx	@dptr,a
00102$:
;	main.c:455: if(io_counter%2==0){io_cnt(io_counter/2);}
;	genAssign
	mov	dptr,#_io_counter
	movx	a,@dptr
	mov	r2,a
...
;------------------------------------------------------------
;	main.c:470: void timer_isr() interrupt 1
;	-----------------------------------------
;	 function timer_isr
;	-----------------------------------------
_timer_isr:
...
;	Peephole 236.a	used r2 instead of ar2
	add	a,r2
	movx	@dptr,a
;	main.c:474: timers=cnt[0]+cnt[1]+cnt[2];            // Calculate how many counters are running curently
;	genPointerGet
;	genFarPointerGet
	mov	dptr,#_cnt
	movx	a,@dptr
...
;	Peephole 236.b	used r3 instead of ar3
	...
	inc	dptr
	movx	@dptr,a
;	main.c:476: if(flag==2)                             // Interrupt occurs every 50 ms , so for 100ms accuracy flag is compared with 2
;	genAssign
	mov	dptr,#_flag
	movx	a,@dptr
	mov	r2,a
...
00149$:
	ljmp	00118$
00150$:
;	main.c:479: clock_control();                    // Clock control for displaying clock
;	genCall
...
;	genIfxJump
	...
	ljmp	00118$
00151$:
;	main.c:485: if(cnt[i]==1)                              // Checking status of 3 counters, IF they are enabed then start decrementing the counter
;	genPlus
;	Peephole 236.g	used r2 instead of ar2
	mov	a,r2
	add	a,#_cnt
...
00152$:
	ljmp	00130$
00153$:
;	main.c:488: if(nms[i]==0)                           // decreament millisec from 10 to 0
;	genLeftShift
;	genLeftShiftLiteral
;	genlshTwo
	mov	ar4,r2
...
;	Peephole 181	changed mov to clr
	clr	a
	movx	@dptr,a
;	main.c:491: if(nsec[i]>0){nsec[i]--;}else{nsec[i]=0;}
;	genPlus
;	Peephole 236.g	used r4 instead of ar4
	mov	a,r4
	add	a,#_nsec
...
;	main.c:495: nsec[i]=59;                            // decreament sec from 59 to 0
;	genPointerSet
;     genFarPointerSet
	mov	dpl,r6
	mov	dph,r7
...
;	Peephole 112.b	changed ljmp to sjmp
	sjmp	00110$
00107$:
;	main.c:497: else{nmi[i]=0;nms[i]=0; nsec[i]=0;}
;	genPointerSet
;     genFarPointerSet
	mov	dpl,r0
	mov	dph,r1
...
;	Peephole 101	removed redundant mov
	movx	@dptr,a
00110$:
;	main.c:499: if(nmi[i]==0 &&nms[i]==0 && nsec[i]==0)      // If all of them reaches to zero then disble the counter and display the alarm
;	genPlus
;	Peephole 236.g	used r4 instead of ar4
	mov	a,r4
	add	a,#_nmi
...
;	Peephole 181	changed mov to clr
	...
	movx	@dptr,a
00112$:
;	main.c:503: countdown_alarm(nms[i],nsec[i],nmi[i],cnt[i],i);
;	genPlus
;	Peephole 236.g	used r4 instead of ar4
	mov	a,r4
	add	a,#_nms
...
00163$:
	ljmp	00128$
00118$:
;	main.c:511: if(timers==3)                                   // Dynamically adjust the timing of the interrupt by keeping track of how many timers are active currently
;	genAssign
	mov	dptr,#_timers
	movx	a,@dptr
	mov	r2,a
...
;	Peephole 112.b	changed ljmp to sjmp
	sjmp	00132$
00126$:
;	main.c:513: else if(timers==2){TH0 =   0xB5; TL0 =   0x00;}
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.a	optimized misc jump sequence
...
;	Peephole 112.b	changed ljmp to sjmp
	sjmp	00132$
00123$:
;	main.c:514: else if(timers==1){TH0 =   0x90;  TL0 =   0x00;}
;	genCmpEq
;	gencjneshort
;	Peephole 112.b	changed ljmp to sjmp
;	Peephole 198.a	optimized misc jump sequence
...
__str_8:
	...
	.db 0x09
	.db 0x09
	.ascii "'t'EEPROM Time Measure"
	.db 0x0A
	.db 0x0D
	.db 0x09
	.ascii "'b"
	.ascii "'. EEPROM Block Fill "
	.db 0x00
__str_9:
	.db 0x0A
	.db 0x0A
